...
import { setSelectedProvider } from '../services/geminiService';
...
const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, onToggleRole, currentRole, onOpenWPDashboard }) => {
    ...
    const [currentProvider, setCurrentProvider] = useState('poyo1');
            ...
            <div className="bg-white dark:bg-brand-blue rounded-xl shadow-2xl w-full max-w-md m-4 overflow-hidden border border-brand-gold/30" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-6 space-y-8 max-h-[80vh] overflow-y-auto">
                    ...
                    <section className="space-y-4">
                        <h3 className="text-sm font-bold text-brand-gold uppercase tracking-wider mb-2">AI Provider</h3>
                        <div className="bg-gray-100 dark:bg-gray-800 p-3 rounded-lg border border-gray-200 dark:border-gray-700">
                            <p className="text-xs text-gray-500 mb-2">Select the AI engine for responses and drafting.</p>
                            <select 
                                value={currentProvider} 
                                onChange={(e) => handleProviderChange(e.target.value)}
                                className="w-full bg-white dark:bg-brand-blue border border-gray-300 dark:border-brand-gold/30 rounded px-3 py-2 text-sm text-gray-800 dark:text-white outline-none focus:border-brand-gold"
                            >
                                <option value="poyo1">Poyo.ai (Primary)</option>
                                <option value="poyo2">Poyo.ai (Backup)</option>
                                <option value="portkey">Portkey.ai</option>
                                <option value="openrouter">OpenRouter</option>
                                <option value="gemini">Gemini (Replit Native)</option>
                            </select>
                        </div>
                    </section>
