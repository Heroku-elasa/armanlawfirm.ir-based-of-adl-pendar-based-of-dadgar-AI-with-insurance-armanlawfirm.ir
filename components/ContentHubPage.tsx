...
const ContentHubPage: React.FC<ContentHubPageProps> = ({
    ...
    const [publishInterval, setPublishInterval] = useState(3);

    const handlePublishToTelegram = async (postId: string) => {
        try {
            ...
            if (response.ok) {
                addToast("Published to Telegram!", "success");
            } else {
                addToast("Failed to publish to Telegram.", "error");
            }
        } catch (error) {
            addToast("Error publishing to Telegram.", "error");
        }
    };

    const handleToggleAutoPublish = () => {
        setIsAutoPublishEnabled(!isAutoPublishEnabled);
        addToast(`Auto-publish ${!isAutoPublishEnabled ? 'enabled' : 'disabled'}`, "info");
    };
    ...
    return (
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24 animate-fade-in">
            <div className="text-center max-w-4xl mx-auto relative">
                ...
                {/* Telegram & Auto-Publish Controls */}
                <div className="mt-8 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 flex flex-wrap items-center justify-center gap-6">
                    <div className="flex items-center gap-3">
                        <span className="text-2xl">📢</span>
                        <div className="text-left">
                            <h3 className="font-bold text-gray-900 dark:text-white">Telegram Channel</h3>
                            <p className="text-xs text-gray-500">Auto-post generated content</p>
                        </div>
                    </div>
                    
                    <div className="flex items-center gap-4 border-l border-gray-200 dark:border-gray-700 pl-6">
                        <div className="flex flex-col items-start">
                            <label className="text-xs font-bold text-gray-400 uppercase mb-1">Interval (Hours)</label>
                            <input 
                                type="number" 
                                value={publishInterval} 
                                onChange={(e) => setPublishInterval(parseInt(e.target.value))}
                                className="w-16 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded px-2 py-1 text-sm"
                            />
                        </div>
                        
                        <button 
                            onClick={handleToggleAutoPublish}
                            className={`px-4 py-2 rounded-lg font-bold transition-all ${isAutoPublishEnabled ? 'bg-green-500 text-white shadow-lg' : 'bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300'}`}
                        >
                            {isAutoPublishEnabled ? 'Auto-Publish: ON' : 'Auto-Publish: OFF'}
                        </button>
                    </div>

                    <div className="bg-blue-50 dark:bg-blue-900/20 px-4 py-3 rounded-xl border border-blue-100 dark:border-blue-800 text-xs text-blue-700 dark:text-blue-300 max-w-xs text-left">
                        <strong>Cloudflare Setup:</strong> Set <code>TELEGRAM_BOT_TOKEN</code> and <code>TELEGRAM_CHAT_ID</code> in Pages settings. Use Workers Cron Triggers for the 3-hour schedule.
                    </div>
                </div>
