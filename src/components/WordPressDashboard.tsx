...
const WordPressDashboard: React.FC<WordPressDashboardProps> = ({ setPage, userRole }) => {
    ...
    const [selectedProvider, setSelectedProvider] = useState<string>(localStorage.getItem('selectedAIProvider') || 'auto');
                    ...
                    {activeMenu === 'SEO Check' ? (
                        ...
                        <div className="animate-fade-in space-y-6">
                            <div className="bg-white border border-[#c3c4c7] shadow-sm">
                                <div className="px-4 py-3 border-b border-[#c3c4c7] font-semibold text-sm bg-white flex justify-between items-center">
                                    <span>AI Provider Settings</span>
                                    <button 
                                        onClick={loadProvidersStatus}
                                        className="text-[#2271b1] hover:text-[#135e96] text-xs"
                                    >
                                ...
                                <div className="p-4">
                                    <div className="mb-6">
                                        <label className="block text-sm font-medium text-[#1d2327] mb-2">Primary AI Provider</label>
                                        <select 
                                            value={selectedProvider}
                                            onChange={(e) => {
                                                const val = e.target.value;
                                                setSelectedProvider(val);
                                                localStorage.setItem('selectedAIProvider', val);
                                            }}
                                            className="w-full max-w-md border border-[#8c8f94] rounded h-[30px] text-sm px-2 focus:border-[#2271b1] focus:ring-1 focus:ring-[#2271b1] outline-none"
                                        >
                                            <option value="auto">Automatic (Priority-based Fallback)</option>
                                            {providersStatus.map(p => (
                                                <option key={p.key} value={p.key}>{p.name}</option>
                                            ))}
                                        </select>
                                        <p className="mt-2 text-xs text-[#646970]">
                                            Select "Automatic" for high reliability. The system will automatically switch to backup providers if the primary fails.
                                        </p>
                                    </div>

                                    <div className="space-y-3">
                                        <h3 className="text-sm font-medium text-[#1d2327]">Provider Status</h3>
                                        {providersStatus.length === 0 ? (
                                            <p className="text-xs text-[#646970]">Loading provider status...</p>
                                        ) : (
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                {providersStatus.map(p => (
                                                    <div key={p.key} className="flex items-center justify-between p-3 border border-[#dcdcde] rounded bg-[#f6f7f7]">
                                                        <div className="flex items-center gap-3">
                                                            <div className={`w-3 h-3 rounded-full ${p.available ? 'bg-[#008a20]' : 'bg-[#d63638]'}`}></div>
                                                            <div>
                                                                <div className="text-sm font-medium">{p.name}</div>
                                                                <div className="text-[10px] text-[#646970]">Priority: {p.priority}</div>
                                                            </div>
                                                        </div>
                                                        {p.isCurrent && (
                                                            <span className="text-[10px] bg-[#2271b1] text-white px-2 py-0.5 rounded-full">Active</span>
                                                        )}
                                                    </div>
                                                ))}
                                            </div>
